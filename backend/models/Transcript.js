...
// added to two profiles, and each copy would be transcribed and BILLED
// separately for text we already hold. The route turns the resulting
// duplicate-key error into "that video is already in <profile name>", which is
// the honest answer.
TranscriptSchema.index({ user: 1, video_id: 1 }, { unique: true });

// The per-profile queries: slot counts, the video list, and the analysis input.
TranscriptSchema.index({ user: 1, profile: 1, status: 1 });

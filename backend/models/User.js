...
const UserSchema = new Schema({
  ...
  // What this creator's channels cover (the union across their profiles), from
  // the script-writer era. The news collector that scheduled off it and the
  // first-run screen that set it are both gone; kept for existing accounts and
  // the parked profile screens, and read by socket/auth.js.
  categories: { type: [String], default: [], index: true },

  // When the old first-run category screen was completed. Nothing reads it now
  // that the screen is gone; kept so existing rows stay as they are.
  onboarded_at: { type: Date, default: null },

  last_login:   { type: Date, default: Date.now, index: true },

  // Last sign-in, and, while the news feed existed, the last time they opened
  // it. The admin panel still shows it.

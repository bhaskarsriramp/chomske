...
export function sanitizeTimeline(input, { duration = 0, source = null } = {}) {
  ...
  // Quiet stretches the analysis found and deliberately did NOT remove, kept so
  // the editor can offer them rather than the creator losing footage silently.
  out.dead_air = (Array.isArray(src.dead_air) ? src.dead_air : [])
    .slice(0, 200)
    .map((c) => ({
      id: String(c.id || newId("cut")),
      start: round3(clamp(num(c.start), 0, total)),
      end: round3(clamp(num(c.end), 0, total)),
      reason: String(c.reason || "idle").slice(0, 24),
      auto: c.auto !== false,
    }))
